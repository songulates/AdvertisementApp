...
namespace UILayer.Controllers
{
    public class AdvertisementController : Controller
    {
        ...
        {
            //burda userıd aldık, bu string int cevir
            //sonra genderıd ye göre hareket edicez
            var userId = int.Parse((User.Claims.FirstOrDefault(x => x.Type == ClaimTypes.NameIdentifier)).Value);
            //burda user bilgimizde mevcut olcak
        ...
        public async Task<IActionResult> Send(AdvertisementAppUserCreateModel model)
        {
            ...
            if (response.ResponseType == CommonLayer.ResponseType.ValidationError)
            {
